<?php
class ei_cuadro_viaticos extends sicd_ei_cuadro
{
	...
	{
		$nombre_campos = $this->get_nombres_parametros();
		...
		$eventos_multiples = $this->get_nombres_eventos_multiples();
	...
	{
		$this->resetear_claves_enviadas();
		$id_js = $this->get_id_objeto_js();
		$total_col = $this->get_cantidad_columnas_total();
		$muestra_titulo_cc = $this->debe_mostrar_titulos_columnas_cc();
		$cuadro_colapsa = $this->es_cuadro_colapsado();
	...
	{
		$info_cuadro = $this->get_informacion_basica_cuadro();
		//-- Scroll
	...
	function html_fin()
	{
		$acumulador = $this->get_acumulador_general();
		$info_cuadro = $this->get_informacion_basica_cuadro();
		if (isset($acumulador)) {
	...
	protected function html_cabecera()
	{
		...
		if (isset($info_cuadro) && $info_cuadro['exportar_xls'] == 1) {
			...
			if ($this->permite_exportacion_excel_plano()) {
	...
	function html_mensaje_cuadro_vacio($texto)
	{
		$this->resetear_claves_enviadas();
		$info_cuadro = $this->get_informacion_basica_cuadro();
		$colapsado = $this->es_cuadro_colapsado();
		$objeto_js = $this->get_id_objeto_js();
	...
	{
		if($this->get_cortes_modo() == apex_cuadro_cc_anidado){
			echo "<ul>\n";
		}
	}
	...
	*/
	function html_cabecera_corte_control(&$nodo, $id_unico = null)
	{
		...
		if($this->get_cortes_modo() == apex_cuadro_cc_tabular) {
				$objeto_js = $this->get_id_objeto_js();
				$total_columnas = $this->get_cantidad_columnas_total();
	...
	*/
	...
	{
		if($this->get_cortes_modo() == apex_cuadro_cc_tabular){				//MODO TABULAR
			$indice = $this->get_indice_cortes();
			$total_columnas = $this->get_cantidad_columnas_total();
	...
	function html_cuadro(&$filas)
	{
		...
		if (! $this->debe_mostrar_titulos_columnas_cc()) {
			$this->html_cuadro_cabecera_columnas();
		}
		$par = false;
		$formateo = $this->get_instancia_clase_formateo('html');
		$layout_cant_columnas = $this->get_layout_cant_columnas();
		$i = 0;
		...
		$objeto_js = $this->get_id_objeto_js();
		$evt_multiples = $this->get_eventos_multiples();
		...
		foreach($filas as $f)
		{
			...
			$estilo_fila = $par ? 'ei-cuadro-celda-par' : 'ei-cuadro-celda-impar2';
	...
	protected function html_cuadro_celda_evento($id_fila, $clave_fila, $pre_columnas)
	{
		foreach ($this->get_eventos_sobre_fila() as $id => $evento) {
			...
			if ($grafico_evento) {
					...
					if ($evento->posee_accion_respuesta_popup()) {
						...
						echo  toba_form::hidden($this->get_id_form(). $id_fila .'_descripcion', toba_js::sanear_string($this->get_descripcion_resp_popup($id_fila)));	//Podemos hacer esto porque no vuelve nada!
					}
		...
		$this->agregar_clave_enviada($clave_fila);
	}
	...
	protected function get_parametros_interaccion($id_fila, $clave_fila)
	{
		if ($this->usa_modo_seguro()) {
			...
			$parametros =  $this->get_clave_fila_array($id_fila);
		}
		return $parametros;
	}
	...
	protected function html_cuadro_cabecera_columnas()
	{
		...
		if ($alguna_tiene_titulo) {
			...
			$columnas_agrupadas = $this->get_columnas_agrupadas();
			...
			foreach ($this->get_eventos_sobre_fila() as $evento) {
				...
				if ( !($pre_columnas xor $evento->tiene_alineacion_pre_columnas())) {
					...
					if (toba_editor::modo_prueba()) {
						$info_comp = $this->get_informacion_basica_componente();
						echo toba_editor::get_vinculo_evento($this->get_id(), $info_comp['clase_editor_item'], $evento->get_id())."\n";
					}
					echo "</td>\n";
				}
			}
	...
	protected function html_cuadro_cabecera_columna($titulo,$columna,$indice)
	{
		...
		if (	isset($eventos['ordenar'])
			...
			foreach($sentido as $sen){
				$sel="";
				if ($this->es_sentido_ordenamiento_seleccionado($columna, $sen[0])) {
		...
		if ( toba_editor::modo_prueba()) {
			$item_editor = "1000253";
			$param_editor = array( apex_hilo_qs_zona => implode(apex_qs_separador,$this->get_id()),
									'columna' => $columna );
	...
	{
		$sumarizacion = $this->calcular_totales_sumarizacion_usuario();
		$total_columnas = $this->get_cantidad_columnas_total();
	...
	protected function html_cuadro_totales_columnas($totales,$estilo=null,$agregar_titulos=false, $estilo_linea=null)
	{
		$formateo = $this->get_instancia_clase_formateo('html');
		$columnas = $this->get_columnas();
		$cant_evt_fila = $this->cant_eventos_sobre_fila();
	...
	{
		$objeto_js = $this->get_id_objeto_js();
		$total_registros = $this->get_total_registros();
		$tamanio_pagina = $this->get_tamanio_pagina();
		$pagina_actual = $this->get_pagina_actual();
		$cantidad_paginas = $this->get_cantidad_paginas();
		$parametros = $this->get_nombres_parametros();
		$eventos = $this->get_eventos();
	...
	protected function html_barra_total_registros()
	{
		...
		if ($this->debe_mostrar_total_registros()) {
